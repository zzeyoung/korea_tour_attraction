...
export default function CollectionCard({ character, status }: Props) {
  ...
  return (
    ...
    >
      ...
      {/* 상단 우 — 방문 인증 뱃지 */}
      <div className="absolute top-2 right-2 w-6 h-6 bg-emerald-500 text-white rounded-full flex items-center justify-center text-xs font-bold shadow-md ring-2 ring-white">
        ✓
      </div>
      {/* 하단 — 방문 날짜 (그라데이션) */}
      {status.visitedAt && (
