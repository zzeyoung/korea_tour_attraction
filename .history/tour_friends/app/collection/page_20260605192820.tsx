...
import type { CharactersData, CollectionStatus, Character } from '@/lib/types';
import { getAllCollectionStatuses } from '@/lib/storage';
import CollectionCard from '@/components/CollectionCard';
import TopNav from '@/components/TopNav';
import BottomNav from '@/components/BottomNav';
import { parseRegion, getAllProvinces } from '@/lib/region';
...
export default function CollectionPage() {
  ...
  const grouped = useMemo(() => {
    const provinces = getAllProvinces(allCharacters);
    return provinces.map((province) => {
      const list = allCharacters.filter(
        (c) => parseRegion(c.region).province === province
      );
  ...
  return (
    <div className="min-h-screen bg-stone-50 pb-20 md:pb-0">
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8 py-10 md:py-14">
        ...
        {/* 진행도 카드 */}
        <div className="bg-gradient-to-br from-stone-900 to-stone-700 text-white rounded-2xl p-6 md:p-8 mb-10 shadow-lg">
        ...
        <div className="space-y-10">
          {grouped.map(({ province, list }) => {
            const provinceVisited = hydrated
              ? list.filter(
